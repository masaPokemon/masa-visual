...
goog.events.BrowserFeature = {
    ...
    PASSIVE_EVENTS: (0, $jscomp.scope.purify)(function() {
        ...
        goog.global.addEventListener("test", null, b);
        goog.global.removeEventListener("test", null, b);
        return a
    })
};
goog.disposable = {};
        ...
        if (goog.events.CAPTURE_SIMULATION_MODE == goog.events.CaptureSimulationMode.ON) {
            if (!goog.events.isMarkedIeEvent_(d)) {
                ...
                for (c = e.currentTarget; c; c = c.parentNode)
                    d.push(c);
                for (var g = a.type, h = d.length - 1; !e.propagationStopped_ && 0 <= h; h--)
                    e.currentTarget = d[h],
                    c =
                    goog.events.fireListeners_(d[h], g, !0, e),
                    f = f && c;
                for (h = 0; !e.propagationStopped_ && h < d.length; h++)
                    e.currentTarget = d[h],
...
goog.html.TrustedResourceUrl.formatWithParams = function(a, b, c) {
    ...
    b = /\?/.test(a) ? "&" : "?";
    for (var d in c)
        null != c[d] && (a += b + encodeURIComponent(d) + "=" + encodeURIComponent(String(c[d])), b = "&");
...
goog.html.SafeStyle.create = function(a) {
    ...
    for (c in a) {
        ...
        var d = a[c];
        if (null != d) {
            if (d instanceof goog.string.Const)
                d = goog.string.Const.unwrap(d),
                goog.asserts.assert(!/[{;}]/.test(d), "Value does not allow [{;}].");
            else {
                d = String(d);
                var e = d.replace(goog.html.SafeUrl.FUNCTIONS_RE_, "$1").replace(goog.html.SafeUrl.URL_RE_, "url");
                goog.html.SafeStyle.VALUE_RE_.test(e) ? goog.html.SafeStyle.hasBalancedQuotes_(d) ? d = goog.html.SafeStyle.sanitizeUrl_(d) :
                (goog.asserts.fail("String value requires balanced quotes, got: " + d), d = goog.html.SafeStyle.INNOCUOUS_STRING) : (goog.asserts.fail("String value allows only " + goog.html.SafeStyle.VALUE_ALLOWED_CHARS_ + " and simple functions, got: " + d), d = goog.html.SafeStyle.INNOCUOUS_STRING)
            }
            b += c + ":" + d + ";"
        }
    }
...
goog.dom.getElementsByTagNameAndClass_ = function(a, b, c, d) {
    ...
    if (c) {
        ...
        for (f = b = 0; g = d[f]; f++)
            e = g.className,
            "function" == typeof e.split && goog.array.contains(e.split(/\s+/), c) && (a[b++] =
            g);
        a.length = b;
        return a
    }
...
goog.events.KeyHandler.prototype.handleEvent = function(a) {
    ...
    if (goog.userAgent.IE && a.type == goog.events.EventType.KEYPRESS) {
        ...
        (goog.userAgent.WEBKIT || goog.userAgent.EDGE) && a.type == goog.events.EventType.KEYPRESS ? (d = this.keyCode_, e = 0 <= b.charCode && 63232 > b.charCode && goog.events.KeyCodes.isCharacterKey(d) ? b.charCode : 0) : goog.userAgent.OPERA && !goog.userAgent.WEBKIT ? (d = this.keyCode_,
...
goog.structs.Trie.prototype.remove = function(a) {
    ...
    for (delete c.value_; 0 < d.length;)
        if (b = d.pop(), c = b[0], b = b[1], c.childNodes_[b].isEmpty())
            delete c.childNodes_[b];
        else
            break;
    return a
};
...
Blockly.VariableMap.prototype.renameVariable = function(a, b) {
    ...
    a && (d = g.indexOf(a));
    c && (e = g.indexOf(c));
    -1 == d && -1 == e ? (this.createVariable(b, ""), console.log("Tried to rename an non-existent variable.")) : d == e || -1 != d && -1 == e ? (c = this.variableMap_[f][d], Blockly.Events.fire(new Blockly.Events.VarRename(c, b)), c.name = b) : -1 != d && -1 != e && (c = this.variableMap_[f][e], Blockly.Events.fire(new Blockly.Events.VarRename(c, b)),
...
Blockly.Extensions.MUTATOR_PROPERTIES_ = ["domToMutation", "mutationToDom", "compose", "decompose"];
...
Blockly.Extensions.registerMutator = function(a, b, c, d) {
    ...
    Blockly.Extensions.checkHasFunction_(e, b, "domToMutation");
    Blockly.Extensions.checkHasFunction_(e, b, "mutationToDom");
...
Blockly.Extensions.apply = function(a, b, c) {
    ...
    e.apply(b);
    if (c)
        Blockly.Extensions.checkBlockHasMutatorProperties_(a, b, 'Error after applying mutator "' + a + '": ');
    else if (!Blockly.Extensions.mutatorPropertiesMatch_(d, b))
...
Blockly.Extensions.checkNoMutatorProperties_ = function(a, b) {
    for (var c = 0; c < Blockly.Extensions.MUTATOR_PROPERTIES_.length; c++) {
        ...
        if (b.hasOwnProperty(d))
            throw Error('Error: tried to apply mutation "' + a + '" to a block that already has a "' + d + '" function.  Block id: ' + b.id);
    }
};
...
Blockly.Extensions.checkBlockHasMutatorProperties_ = function(a, b) {
    if (!b.hasOwnProperty("domToMutation"))
        throw Error(a + 'Applying a mutator didn\'t add "domToMutation"');
    if (!b.hasOwnProperty("mutationToDom"))
...
Blockly.Extensions.getMutatorProperties_ = function(a) {
    for (var b = [], c = 0; c < Blockly.Extensions.MUTATOR_PROPERTIES_.length; c++)
        b.push(a[Blockly.Extensions.MUTATOR_PROPERTIES_[c]]);
    return b
};
...
Blockly.Gesture.prototype.updateIsDraggingWorkspace_ = function() {
    if (this.flyout_ ? this.flyout_.isScrollable() : this.startWorkspace_ && this.startWorkspace_.isDraggable())
        this.workspaceDragger_ = this.flyout_ ? new Blockly.FlyoutDragger(this.flyout_) : new Blockly.WorkspaceDragger(this.startWorkspace_),
...
Blockly.utils.tokenizeInterpolation_ = function(a, b) {
    ...
    for (var e = 0, f = [], g = null, h = 0; h < d.length; h++) {
        var k = d[h];
        0 == e ? "%" == k ? ((k = f.join("")) && c.push(k), f.length = 0, e = 1) : f.push(k) : 1 == e ? "%" == k ? (f.push(k), e = 0) : b && "0" <= k && "9" >= k ? (e = 2, g = k, (k = f.join("")) && c.push(k), f.length = 0) : "{" == k ? e = 3 : (f.push("%", k), e = 0) : 2 == e ? "0" <= k && "9" >= k ? g += k : (c.push(parseInt(g, 10)), h--, e = 0) : 3 == e && ("" == k ? (f.splice(0, 0, "%{"), h--, e = 0) : "}" != k ? f.push(k) : (k = f.join(""), /[a-zA-Z][a-zA-Z0-9_]*/.test(k) ? (e = k.toUpperCase(),
        (e = goog.string.startsWith(e, "BKY_") ? e.substring(4) : null) && e in Blockly.Msg ? (k = Blockly.Msg[e], goog.isString(k) ? Array.prototype.push.apply(c, Blockly.utils.tokenizeInterpolation(k)) : b ? c.push(String(k)) : c.push(k)) : c.push("%{" + k + "}")) : c.push("%{" + k + "}"), e = f.length = 0))
    }
    (k = f.join("")) && c.push(k);
    d = [];
...
Blockly.Options = function(a) {
    ...
    if (g) {
        ...
        h = Blockly.Options.parseToolboxTree(a.toolbox),
    ...
    n = "end" === n ? !1 : !0;
    var p = m ? n ? Blockly.TOOLBOX_AT_TOP : Blockly.TOOLBOX_AT_BOTTOM :
    n == l ? Blockly.TOOLBOX_AT_RIGHT : Blockly.TOOLBOX_AT_LEFT;
    var q = a.scrollbars;
    void 0 === q && (q = f);
    var t = a.css;
    void 0 === t && (t = !0);
...
Blockly.Xml.blockToDom = function(a, b) {
    var c,
        d = goog.dom.createDom(a.isShadow() ? "shadow" : "block");
    ...
    for (var f; f = a.inputList[e]; e++) {
        ...
        for (var h; h = f.fieldRow[g]; g++) {
            var k = h;
            k.name && k.EDITABLE && (h = goog.dom.createDom("field", null, k.getValue()), h.setAttribute("name", k.name), k instanceof Blockly.FieldVariable && (k = a.workspace.getVariable(k.getValue())) &&
            (h.setAttribute("id", k.getId()), h.setAttribute("variableType", k.type)), d.appendChild(h))
        }
    }
    if (e = a.getCommentText())
        e = goog.dom.createDom("comment", null, e),
    ...
    for (e = 0; f = a.inputList[e]; e++)
        h = !0,
        f.type != Blockly.DUMMY_INPUT && (k = f.connection.targetBlock(), f.type ==
        Blockly.INPUT_VALUE ? c = goog.dom.createDom("value") : f.type == Blockly.NEXT_STATEMENT && (c = goog.dom.createDom("statement")), g = f.connection.getShadowDom(), !g || k && k.isShadow() || c.appendChild(Blockly.Xml.cloneShadow_(g)), k && (c.appendChild(Blockly.Xml.blockToDom(k, b)), h = !1), c.setAttribute("name", f.name), h || d.appendChild(c));
    ...
    a.isDeletable() ||
    a.isShadow() || d.setAttribute("deletable", !1);
    a.isMovable() || a.isShadow() || d.setAttribute("movable", !1);
    a.isEditable() || d.setAttribute("editable", !1);
    if (e = a.getNextBlock())
        c = goog.dom.createDom("next", null, Blockly.Xml.blockToDom(e, b)),
        d.appendChild(c);
    g = a.nextConnection && a.nextConnection.getShadowDom();
    !g || e && e.isShadow() || c.appendChild(Blockly.Xml.cloneShadow_(g));
    return d
};
    ...
    for (var f = null, g = 0, h; h = a.childNodes[g]; g++)
        if (3 != h.nodeType) {
            var k = null,
                l = null;
            f = 0;
            for (var m; m = h.childNodes[f]; f++)
                1 == m.nodeType && ("block" == m.nodeName.toLowerCase() ? k = m : "shadow" == m.nodeName.toLowerCase() && (l = m));
            !k && l && (k = l);
            ...
            case "field":
                ...
                Blockly.FieldVariable) {
                    ...
                    if (null !== m && m !== p.type)
                        throw Error("Serialized variable type with id '" + p.getId() + "' had type " + p.type + ", and does not match variable field that references it: " + Blockly.Xml.domToText(h) + ".");
                }
                if (!k) {
    ...
    if (this.rendered && !(a.getElementsByTagName("block").length >= this.remainingCapacity())) {
        ...
        try {
            ...
            if (goog.userAgent.IE || goog.userAgent.EDGE)
                for (var c = b.getDescendants(), d = c.length - 1; 0 <= d; d--)
                    c[d].render(!1);
            ...
            if (!isNaN(e) && !isNaN(f)) {
                ...
                do {
                    ...
                    if (!g) {
                        var m = b.getConnections_(!1);
                        d = 0;
                        for (var n; n = m[d]; d++)
                            if (n.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(e, f)).connection) {
                                g = !0;
                                break
                            }
                    }
...
Blockly.Mutator.prototype.createEditor_ = function() {
    ...
    if (this.quarkNames_.length) {
        ...
        for (var b = 0, c; c = this.quarkNames_[b]; b++)
...
Blockly.Mutator.prototype.workspaceChanged_ = function() {
    ...
    if (!this.workspace_.isDragging())
        for (var b = this.workspace_.getTopBlocks(!1), c = 0; a = b[c]; c++) {
            var d = a.getRelativeToSurfaceXY(),
                e = a.getHeightWidth();
...
Blockly.Block.prototype.interpolate_ = function(a, b, c) {
    ...
    if (f != b.length)
        throw Error('Block "' + this.type + '": Message does not reference all ' + b.length +
        " arg(s).");
    ...
    for (g = 0; g < a.length; g++)
        if (f = a[g], "string" == typeof f)
            b.push([f, void 0]);
        else {
            d = h = null;
            do if (e = !1, "string" == typeof f)
                h = new Blockly.FieldLabel(f);
            else
                switch (f.type) {
                case "input_value":
...
Blockly.Block.newFieldVariableFromJson_ = function(a) {
    a = Blockly.utils.replaceMessageReferences(a.variable);
    return new Blockly.FieldVariable(a)
};
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.getInputsInline() && !this.isCollapsed(), m = 0, n; n = b[m]; m++)
        if (n.isVisible()) {
            if (l && k && k != Blockly.NEXT_STATEMENT && n.type != Blockly.NEXT_STATEMENT)
                var p = c[c.length - 1];
            else
                k = n.type,
                p = [],
                p.type = l &&
                n.type != Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : n.type,
                p.height = 0,
                c.push(p);
            p.push(n);
            ...
            l || m != b.length - 1 ? !l && n.type == Blockly.INPUT_VALUE && b[m + 1] && b[m + 1].type ==
            Blockly.NEXT_STATEMENT && n.renderHeight-- : n.renderHeight--;
            p.height = Math.max(p.height, n.renderHeight);
            n.fieldWidth = 0;
            1 == c.length && (n.fieldWidth += this.RTL ? -a : a);
            q = !1;
            for (var t = 0, r; r = n.fieldRow[t]; t++) {
            ...
            (g = !0, e = Math.max(e, n.fieldWidth)) : (p.type == Blockly.INPUT_VALUE ? f = !0 : p.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d, n.fieldWidth)))
        }
    for (a = 0; p = c[a]; a++)
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(a, b, c, d, e, f) {
    for (var g, h, k, l, m = 0, n, p = 0, q; q = e[p]; p++) {
        ...
        if (this.isCollapsed())
            ...
            for (g = 0; l = q[g]; g++)
                ...
                l.type == Blockly.INPUT_VALUE && (c.push("M", h - Blockly.BlockSvg.SEP_SPACE_X + "," + (m + Blockly.BlockSvg.INLINE_PADDING_Y)), c.push("h", Blockly.BlockSvg.TAB_WIDTH - 2 - l.renderWidth), c.push(Blockly.BlockSvg.TAB_PATH_DOWN), c.push("v", l.renderHeight + 1 - Blockly.BlockSvg.TAB_HEIGHT), c.push("h",
            ...
            p == e.length - 1 || e[p + 1].type == Blockly.NEXT_STATEMENT) && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), this.RTL && b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), m += Blockly.BlockSvg.SEP_SPACE_Y);
        m += q.height
    }
...
Blockly.Events.filter = function(a, b) {
    ...
    for (var d = 0, e; e = c[d]; d++)
        for (var f = d + 1, g; g = c[f]; f++)
            e.type == g.type && e.blockId == g.blockId && e.workspaceId == g.workspaceId && (e.type == Blockly.Events.MOVE ? (e.newParentId = g.newParentId, e.newInputName = g.newInputName, e.newCoordinate = g.newCoordinate, c.splice(f, 1), f--) : e.type == Blockly.Events.CHANGE && e.element == g.element && e.name == g.name ? (e.newValue = g.newValue, c.splice(f, 1), f--) : e.type != Blockly.Events.UI || "click" != g.element || "commentOpen" !=
...
Blockly.FieldTextInput.prototype.showEditor_ = function(a) {
    ...
    if (!a && (goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD)) {
        ...
        d.onKeyPressWrapper_ = Blockly.bindEventWithChecks_(d,
...
Blockly.FieldDropdown.prototype.showEditor_ = function() {
    ...
    goog.events.listen(b,
    goog.ui.Component.EventType.ACTION, function(b) {
        if (b = b.target)
...
Blockly.FieldDropdown.prototype.trimOptions_ = function() {
    ...
    if (goog.isArray(a)) {
        ...
        if (!(b || 2 > a.length)) {
            ...
            d = Blockly.utils.commonWordSuffix(e,
            c);
            if ((b || d) && !(c <= b + d)) {
...
Blockly.Variables.createVariable = function(a, b) {
    var c = function(d) {
        Blockly.Variables.promptName(Blockly.Msg.NEW_VARIABLE_TITLE, d, function(d) {
            d ? a.getVariable(d) ? Blockly.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1", d.toLowerCase()), function() {
                c(d)
            }) : Blockly.Procedures.isLegalName_(d, a) ? (a.createVariable(d), b && b(d)) : Blockly.alert(Blockly.Msg.PROCEDURE_ALREADY_EXISTS.replace("%1", d.toLowerCase()), function() {
                c(d)
            }) : b && b(null)
        })
    };
...
Blockly.FieldVariable.dropdownCreate = function() {
    ...
    if (d) {
        a = d.getVariablesOfType("");
        for (var e = 0; e < a.length; e++)
            if (c && goog.string.caseInsensitiveEquals(a[e].name, b)) {
                c = !1;
                break
            }
    }
    c && d && (e = d.createVariable(b), a.push(e));
    a.sort(Blockly.VariableModel.compareByName);
    c = [];
    for (e = 0; e < a.length; e++)
        c[e] = [a[e].name, a[e].getId()];
    c.push([Blockly.Msg.RENAME_VARIABLE, Blockly.RENAME_VARIABLE_ID]);
    Blockly.Msg.DELETE_VARIABLE &&
    c.push([Blockly.Msg.DELETE_VARIABLE.replace("%1", b), Blockly.DELETE_VARIABLE_ID]);
    return c
};
    ...
    if (this.sourceBlock_ && this.sourceBlock_.workspace) {
        ...
        if (e)
            ...
            if (c == Blockly.RENAME_VARIABLE_ID) {
                var g = this.getText();
                Blockly.hideChaff();
                Blockly.Variables.promptName(Blockly.Msg.RENAME_VARIABLE_TITLE.replace("%1", g), g, function(a) {
                    a && d.renameVariable(g, a)
                });
                return
            }
...
Blockly.FlyoutButton.prototype.createDom = function() {
    var a,
        b = this.isLabel_ ? "blocklyFlyoutLabel" : "blocklyFlyoutButton";
...
Blockly.Flyout.prototype.clearOldBlocks_ = function() {
    ...
    for (b = 0; a = this.backgroundButtons_[b]; b++)
...
Blockly.HorizontalFlyout.prototype.layout_ = function(a, b) {
    ...
    for (var f = 0, g; g = a[f]; f++)
        if ("block" == g.type) {
            g = g.block;
            for (var h = g.getDescendants(), k = 0; c = h[k]; k++)
                c.isInFlyout = !0;
            g.render();
            h = g.getSvgRoot();
            k = g.getHeightWidth();
            ...
            e += k.width + b[f];
            this.addBlockListeners_(h,
            g, c)
        } else
            "button" == g.type && (this.initFlyoutButton_(g.button, e, d), e += g.button.width + b[f])
};
...
Blockly.HorizontalFlyout.prototype.placeNewBlock_ = function(a) {
    var b = this.targetWorkspace_;
    var c = a.getSvgRoot();
    if (!c)
        throw "originBlock is not rendered.";
    c = b.isMutator ? this.workspace_.getSvgXY(c) : Blockly.utils.getInjectionDivXY_(c);
    var d = this.workspace_.scrollX;
    var e = this.workspace_.scale;
    c.x += d / e - d;
    d = this.workspace_.scrollY;
    e = this.workspace_.scale;
    c.y += d / e - d;
    this.toolboxPosition_ == Blockly.TOOLBOX_AT_BOTTOM && (d = b.getMetrics().viewHeight - this.height_, e = b.scale, c.y += d / e - d);
...
Blockly.VerticalFlyout.prototype.placeNewBlock_ = function(a) {
    var b = this.targetWorkspace_;
    var c = a.getSvgRoot();
    if (!c)
        throw "originBlock is not rendered.";
    ...
    c.x += d / e - d;
    d = b.getMetrics();
    this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT && (d = d.viewWidth - this.width_, c.x += d / e - d);
    d = this.workspace_.scrollY;
    e = this.workspace_.scale;
    c.y += d / e - d;
    a = Blockly.Xml.blockToDom(a);
    a = Blockly.Xml.domToBlock(a,
    b);
    e = a.getSvgRoot();
    if (!e)
        throw "block is not rendered.";
    ...
    b.readOnly || b.hasScrollbars || e.addChangeListener(function() {
        if (!e.isDragging()) {
            ...
            a.contentHeight > a.viewHeight + d || a.contentLeft < (b.RTL ? a.viewLeft : c) || a.contentLeft + a.contentWidth > (b.RTL ? a.viewWidth : a.viewWidth + c))
                for (var k = e.getTopBlocks(!1), l = 0, m; m = k[l]; l++) {
...
Blockly.bindEventWithChecks_ = function(a, b, c, d, e) {
    ...
    if (b in Blockly.Touch.TOUCH_MAP)
        for (var k = function(a) {
                g(a);
                f && a.preventDefault()
            }, l = 0, m; m = Blockly.Touch.TOUCH_MAP[b][l]; l++)
            a.addEventListener(m, k, !1),
            h.push([a, m, k]);
    return h
};
